...
/**
 * A handle for a parsed JSON schema.
 *
 * @category Schema
 */
export class Schema<T = unknown> {
  ...
  /** @internal */
  constructor(node: T, context: SchemaContext) {
    this.node = node;
 ...
 * @category Schema
 */
export async function parseSchema<const T>(
  node: T,
 ...
 * @internal
 */
const dialects: ReadonlyMap<string, Dialect> = new Map<string, Dialect>([
